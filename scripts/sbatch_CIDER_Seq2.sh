#!/bin/bash
#SBATCH --job-name=CIDER_Seq2
#SBATCH --output=logs_ciderseq2/%x_%A_%a.out
#SBATCH --error=logs_ciderseq2/%x_%A_%a.err
#SBATCH --cpus-per-task=16
#SBATCH --mem=100G
#SBATCH --time=96:00:00
#SBATCH --array=0-26

# CIDER-Seq2 批量运行脚本 (SLURM array job)
# 用于 PacBio HiFi 长读长数据的 eccDNA 检测 (RCA + SMRT)
...
# 日志目录
LOG_DIR="${BASE_DIR}/logs_ciderseq2"
mkdir -p ${LOG_DIR}

# 汇总统计文件
STATS_FILE="${LOG_DIR}/ciderseq2_benchmark_stats.tsv"
...
# 检查是否已完成
if [ -f "${OUTPUT_DIR}/${PREFIX}/02_eccDNA/output/${PREFIX}_eccDNA.bed" ]; then
    echo "[已完成] ${PREFIX}"
    exit 0
fi
